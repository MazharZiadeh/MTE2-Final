...
// Constructor: Initializes object with given position, velocity, and radius.
Particles::Particles(glm::vec3 pos, glm::vec3 vel, float rad)
    : position(pos), velocity(vel), radius(rad) {}

// Update object position based on its velocity and the given deltaTime.
...
void Particles::handleParticlesCollisions(Particles& other) {
    // Calculate the vector between the two object positions
